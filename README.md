...
KS thresholded on the D statistic. Rejected: p-value threshold. With n in the hundreds per window, KS p-values go significant on trivial, practically meaningless differences. Thresholding on D directly keeps the alarm tied to effect size instead of sample size. KS D threshold is .15.
